...
describe ImportFileProcessor do
  ...
  describe '.run' do
    ...
    it 'should read file and create records' do
      ...
      parser.should_receive(:process).with(content).and_return(parser)
      parser.should_receive(:results).and_return(results)

      model = double(:model)
      model.should_receive(:create!).with(results, :transaction)

      processor.should_receive(:calculate_key_for_content)
  ...
  describe '.calculate_key_for_content' do
    subject { processor }

    its(:calculate_key_for_content) do
